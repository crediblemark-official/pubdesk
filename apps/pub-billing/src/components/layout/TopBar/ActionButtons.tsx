...
export const ActionButtons: React.FC<ActionButtonsProps> = ({ activeModule }) => {
  ...
  const handleRefresh = async () => {
    setRefreshing(true);
    showToast('Menyegarkan data invoice & master...', 'info');
    try {
      await Promise.all([
        loadInvoices(),
        loadContacts(),
      ...
      showToast('Data invoice & master berhasil disegarkan!', 'success');
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="top-bar-gnome-actions">
      <button 
        ...
        disabled={refreshing}
        title={refreshing ? "Sedang menyegarkan data..." : "Segarkan data invoice & master"}
