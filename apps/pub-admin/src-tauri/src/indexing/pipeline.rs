...
pub fn run_indexing_pipeline(app_handle: &AppHandle, file_id: i64) -> Result<(), String> {
    ...
    // 3. Simpan hasil analisis ke DB (lock db singkat hanya saat menulis data)
    {
        ...
        for entity in &entities {
                ...
                match stmt_rel {
                    Ok(mut s) => {
                        if let Ok(rows) = s.query_map(params![entity.0, entity.1, file_id], |r| r.get::<_, i64>(0)) {
                            ...
                            rows.flatten().map(|id| (id, rel_type)).collect()
                        } else {
                            Vec::new()
                        }
                    }
            ...
            for (other_file_id, rel_type) in related_ids {
                let _ = tx.execute(
                    "INSERT OR IGNORE INTO file_relations (source_file_id, target_file_id, relation_type, confidence) VALUES (?1, ?2, ?3, ?4)",
        ...
        let cross_relations: Vec<(i64, f32)> = {
            let stmt_all = tx.prepare("SELECT id, path, filename FROM files WHERE id != ?1");
            match stmt_all {
                Ok(mut s) => {
                    if let Ok(all_rows) = s.query_map(params![file_id], |row| {
                        let id: i64 = row.get(0)?;
                        let other_path: String = row.get(1)?;
                        let other_filename: String = row.get(2)?;
                        Ok((id, other_path, other_filename))
                    }) {
                        all_rows.flatten().filter_map(|(other_id, other_path_str, _other_filename)| {
        ...
        for (other_id, confidence) in cross_relations {
            let _ = tx.execute(
                "INSERT INTO file_relations (source_file_id, target_file_id, relation_type, confidence) VALUES (?1, ?2, ?3, ?4)",
