...
const ProduksiApproval: React.FC = () => {
  ...
  const uniqueJenis = Array.from(new Set(tasks.map(t => t.step_name).filter(Boolean)));
  ...
  return (
    <div className="module-content" style={{ padding: '24px', display: 'flex', flexDirection: 'column', height: '100%', overflow: 'hidden' }}>
      ...
      <div style={{ display: 'flex', gap: '12px', marginBottom: '24px', flexWrap: 'wrap' }}>
        <select value={filterJenis} onChange={e => setFilterJenis(e.target.value)} style={{ padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text-primary)' }}>
          <option value="">[Jenis Approval]</option>
          {uniqueJenis.map(jenis => <option key={jenis as string} value={jenis as string}>{jenis}</option>)}
        </select>
        ...
        <select value={filterTanggal} onChange={e => setFilterTanggal(e.target.value)} style={{ padding: '8px 12px', borderRadius: '6px', border: '1px solid var(--border)', background: 'var(--bg-card)', color: 'var(--text-primary)' }}>
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', flexDirection: 'column', flex: 1, overflow: 'hidden' }}>
        <div style={{ padding: '16px 20px', borderBottom: '1px solid var(--border)', display: 'grid', gridTemplateColumns: '2fr 1fr 1fr 1fr 100px', gap: '16px', fontWeight: '600', color: 'var(--text-secondary)', fontSize: '13px' }}>
          <div>Judul</div>
          <div>Approval</div>
        ...
        <div style={{ overflowY: 'auto', flex: 1 }}>
          {isLoading ? (
            ...
            <div style={{ padding: '40px', textAlign: 'center', color: 'var(--text-secondary)' }}>Tidak ada data yang butuh approval.</div>
          ) : (
